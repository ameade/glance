...
if not hasattr(CONF, 'metadata_encryption_key'):
    CONF.register_opt(cfg.StrOpt('metadata_encryption_key', secret=True))
...
def migrate_location_credentials(migrate_engine, to_quoted):
    ...
    if not CONF.metadata_encryption_key:
        msg = _("'metadata_encryption_key' was not specified in the config file."
                "This means that this migration is a NOOP.")
        LOG.warn(msg)
        return
    ...
    images = images_table.select().execute()
...
def decrypt_location(uri):
    LOG.info(uri)
    to_return = crypt.urlsafe_decrypt(CONF.metadata_encryption_key, uri)
    LOG.info(to_return)
    return to_return
...
def fix_uri_credentials(uri, to_quoted):
    ...
    if to_quoted:
        ...
        location._get_credstring = types.MethodType(legacy__get_credstring,
                                                    location)
    try:
        location.parse_uri(decrypt_location(uri))
