...
UUID3 = utils.generate_uuid()
UUID4 = utils.generate_uuid()
...
class TestImagesController(test_utils.BaseTestCase):
    ...
    def test_index_with_invalid_filter_key(self):
        request = unit_test_utils.get_fake_request('/images?blah=blah')
    ...
    def test_index_with_nonexistant_name_filter(self):
        ...
        output = self.controller.index(request, filters={'name': 'blah'})
...
class TestImagesDeserializer(test_utils.BaseTestCase):
    ...
    def test_index_with_boolean_filter(self):
        path = '/images?protected=True'
        request = unit_test_utils.get_fake_request(path)
        output = self.deserializer.index(request)
        self.assertEqual(output['filters']['protected'], True)

    def test_index_with_boolean_filter_false(self):
        path = '/images?protected=FaLsE'
        request = unit_test_utils.get_fake_request(path)
        output = self.deserializer.index(request)
        self.assertEqual(output['filters']['protected'], False)

    def test_index_with_boolean_filter_on_value(self):
        path = '/images?protected=on'
        request = unit_test_utils.get_fake_request(path)
        output = self.deserializer.index(request)
        self.assertEqual(output['filters']['protected'], True)

    def test_index_with_boolean_filter_off_value(self):
        path = '/images?protected=oFF'
        request = unit_test_utils.get_fake_request(path)
        output = self.deserializer.index(request)
        self.assertEqual(output['filters']['protected'], False)

    def test_index_with_boolean_filter_1_value(self):
        path = '/images?protected=1'
        request = unit_test_utils.get_fake_request(path)
        output = self.deserializer.index(request)
        self.assertEqual(output['filters']['protected'], True)

    def test_index_with_boolean_filter_0_value(self):
        path = '/images?protected=0'
        request = unit_test_utils.get_fake_request(path)
        output = self.deserializer.index(request)
        self.assertEqual(output['filters']['protected'], False)

    def test_index_with_boolean_filter_yes_value(self):
        path = '/images?protected=YEs'
        request = unit_test_utils.get_fake_request(path)
        output = self.deserializer.index(request)
        self.assertEqual(output['filters']['protected'], True)

    def test_index_with_boolean_filter_no_value(self):
        path = '/images?protected=no'
        request = unit_test_utils.get_fake_request(path)
        output = self.deserializer.index(request)
        self.assertEqual(output['filters']['protected'], False)

    def test_index_with_invalid_boolean_filter(self):
        path = '/images?protected=YEssir'
        request = unit_test_utils.get_fake_request(path)
        self.assertRaises(webob.exc.HTTPBadRequest,
