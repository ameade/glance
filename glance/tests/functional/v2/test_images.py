...
class TestImages(functional.FunctionalTest):
    ...
    def test_get_images_with_filtering(self):
        ...
        # Filter images using invalid filter key
        path = self._url('/images?blahblahblah=public')
        response = requests.get(path, headers=self._headers())
        self.assertEqual(400, response.status_code)
        ...
        # Filter images with off boolean value
        path = self._url('/images?is_public=off')
        response = requests.get(path, headers=self._headers())
        self.assertEqual(200, response.status_code)
        images = json.loads(response.text)['images']
        self.assertEqual(1, len(images))
